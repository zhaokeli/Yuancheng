...
CDb::CDb(void)
{
	...
	try
	{
		try {
			...
			{
				::MessageBox(NULL, _T("Create Instance failed!"), _T("error"), MB_ICONEXCLAMATION);
				return;
			}
			...
			{
				::MessageBox(NULL, _T("database connect fail"), _T("error"),0);
				return;
			}
		}
		catch (_com_error &e) {
			::MessageBox(NULL, e.Description(), _T("error"), MB_ICONEXCLAMATION);
		}
	}
	catch (...)
	{
